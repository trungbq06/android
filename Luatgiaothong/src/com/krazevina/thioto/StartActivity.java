...
public class StartActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		DB_PATH = "/data/data/" + this.getApplicationContext().getPackageName()
				+ "/databases/";
		if(!Locale.getDefault().getCountry().toUpperCase().startsWith("VN"))
		{
			Toast.makeText(StartActivity.this, R.string.onlyviet, 1).show();
		}
...
protected class Runprogram extends AsyncTask<Void, Void, Void> {
	...
	protected class LoadingData extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			try{
				cauhoiDB = new CauhoiDB(StartActivity.this);
				getDataBienbao();
				getDataCauhoithi1();
				getDataUseMeothi();
	...
	public void getDataUseMeothi() {
		...
		c.moveToFirst();
		while (!c.isAfterLast()) {
			String keyRequest = c.getString(c.getColumnIndex(CauhoiDB.KEYREQUEST));
	...
	private void getDataCauhoithi1() {
		...
		{ // da co du lieu
			...
			while (!c.isAfterLast()) {
				...
				String phanloai = c.getString(c.getColumnIndex(CauhoiDB.cPHANLOAI));
				String thi = c.getString(c.getColumnIndex(CauhoiDB.cTHI));
				String datraloidapdan = c.getString(c.getColumnIndex(CauhoiDB.cDatraloidapan));
				item = new ItemCauhoi();
